...
The UI shows how RXsignal turns a Prompt Opinion-compatible compact decision-support payload into a clinician-facing controlled-substance workflow.
...
1. The screen opens on a synthetic eCW-style medication page for Tamera164 Wisozk929.
...
3. The medication search shows `Xanax` with two result options:
   - `Xanax 1 mg tablet`
   - `Xanax 0.5 mg tablet`
4. Selecting **Xanax 1 mg tablet** auto-triggers RXsignal because it is a controlled-medication prescribing moment.
5. The UI shows a short transition:
   - controlled medication selected
   - local synthetic PDMP evidence check
   - RXsignal recommendation preparation
6. The RXsignal results modal renders:
   ...
   - workflow buttons
7. After the first automatic review, the pending prescription row exposes **Re-run RXsignal** so the provider can manually repeat the safety check without leaving the workflow.
...
1. Open the RXsignal staging/local UI on the medication page.
2. Click **+ Add Medication** and show the `Xanax` medication search.
3. Select **Xanax 1 mg tablet**.
4. Let the automatic RXsignal transition and results modal load.
5. Pause briefly on the risk score, recommendation, documentation, and workflow buttons.
6. Show **Re-run RXsignal** beside the pending Xanax prescription as the manual provider-control option.
...
- RXsignal is clinician-support decision support, not an autonomous prescriber.
