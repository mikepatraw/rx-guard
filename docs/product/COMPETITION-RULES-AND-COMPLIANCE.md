# Agents Assemble Competition Rules and RXsignal Compliance Review

This document summarizes the rules, requirements, and judging expectations for **Agents Assemble: The Healthcare AI Endgame** and reviews RXsignal’s current repository state against them.
...
## RXsignal chosen path

RXsignal is framed as an **A2A healthcare agent**, not an MCP-superpower-first submission.

Current repo evidence:

- README frames RXsignal as a “Prompt Opinion-published, A2A-enabled prescribing safety agent.”
...
- `src/cli/local-adapter.ts` maps realistic synthetic encounter fields to the Tamera Prompt Opinion patient scope / `PO_PATIENT_TAMERA_WISOZK` overlay and produces the compact Prompt Opinion-compatible payload/response contract.
...
| Requirement / expectation | Current RXsignal status | Assessment |
| --- | --- | --- |
| Use Prompt Opinion platform | Repo docs state RXsignal is published in Prompt Opinion Marketplace and A2A-enabled; staging docs explain the Prompt Opinion-compatible handoff. | **Appears aligned**, but final in-platform invocation should still be recorded before submission. |
| Choose MCP Superpower or A2A Agent path | RXsignal clearly chooses the A2A Agent path. | **Aligned.** |
| Marketplace publication | README and checklist say RXsignal has been published. | **Appears aligned**, pending final polished marketplace URL/details for submission materials. |
...
| AI Factor | Docs describe Prompt Opinion as the agent layer and RXsignal as the clinical data/UI renderer. Local code keeps deterministic fallback output for reliable staging. | **Partially aligned.** The submission should explicitly show Prompt Opinion producing the compact decision-support payload so judges see the model-backed layer. |
| Human-in-the-loop safety | README and architecture explicitly say RXsignal is not an autonomous prescriber and does not replace clinician judgment. | **Aligned.** |
...
- RXsignal targets a specific healthcare workflow: controlled-substance prescribing safety and documentation review.
...
- The public staging UI starts from medication selection, auto-triggers RXsignal for the controlled-medication case, and hides raw JSON/payload details from partner testers.
...
4. **Keep the database story scoped.** Do not build a live prescription/PDMP database before submission. RXsignal should describe production data access as future authorized EHR/PDMP/FHIR integration through a secure clinical data layer.
...
6. **Keep FHIR claims precise.** RXsignal is currently FHIR-aware / FHIR-inspired. Avoid claiming full FHIR server integration unless that is added and validated.

## Recommended competition positioning

RXsignal should be submitted as:

> A Prompt Opinion-compatible A2A healthcare agent workflow for synthetic controlled-substance prescribing review. Prompt Opinion provides the compact decision-support agent output; RXsignal provides the synthetic clinical data adapter and EHR-style workflow renderer that turns that output into clinician-readable risk guidance before a prescription is finalized.

Avoid positioning RXsignal as:
...
RXsignal is **directionally within the competition guidelines** and is well aligned with the A2A Agent path, synthetic-data posture, human-in-the-loop safety framing, and marketplace-oriented workflow.
...
The safest path is to present the current implementation as a narrow, feasible, synthetic-data MVP: Prompt Opinion for the published agent/decision-support layer, RXsignal for the synthetic data adapter and EHR-style workflow renderer, and explicit clinician-review guardrails throughout.
