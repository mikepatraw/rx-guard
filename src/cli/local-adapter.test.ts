...
assert.equal(parsed.name, 'Sheila Bankston');
assert.equal(parsed.dob, '1960-06-13');
...
assert.equal(review.syntheticPatientKey, 'RXG-SB-001');
assert.equal(review.promptOpinionPayload.synthetic_patient_key, 'RXG-SB-001');
...
assert.match(output, /Resolved synthetic key: RXG-SB-001/);
assert.match(output, /Prompt Opinion-safe payload/);
assert.match(output, /"synthetic_patient_key": "RXG-SB-001"/);
assert.doesNotMatch(output, /"name": "Sheila Bankston"/);
assert.doesNotMatch(output, /"dob": "1960-06-13"/);
